    ...
    case "verifyEmail":
      ...
      try {
        ...
        if (!/^[0-9]{6}$/.test(code)) {
          return res
            .status(400)
            .json({ success: false, process: "invalid_code" });
        }
        ...
        if (!verificationRecord) {
          return res
            .status(404)
            .json({ success: false, process: "email_did_not_found" });
        }
        ...
        if (!isMatch) {
          return res
            .status(400)
            .json({ success: false, process: "code_did_not_match" });
        }
        ...
        if (formData.emailList === true) {
          ...
          try {
            await axios.post(
              `${process.env.API_ORIGIN}/api/auth/registerEmail`,
        ...
        const orderDetailsMail = await mailer(
          "orderDetails",
          { validateOrderData, formData },
          res
        );
        const newOrderRecievedMail = await mailer(
          "newOrderRecieved",
          { validateOrderData },
          res
        );

        return createdOrder && orderDetailsMail && newOrderRecievedMail
          ? res.status(200).json({ success: true, message: "verificated" })
          : res
              .status(500)
              .json({ success: false, message: "order couldn't created" });
