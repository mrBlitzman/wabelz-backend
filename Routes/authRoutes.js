...
router.get("/auth/delistEmail", async (req, res) => {
  ...
  try {
      ...
      if (existingEmail.delisted) {
          return res.send('<h1>You are already unsubscribed. You are not a subscriber.</h1>');
      }

      await EmailList.updateOne({ email }, { delisted: true });
      return res.send('<h1>Your email has been unsubscribed successfully. Thank you!</h1>');
  } catch (error) {
