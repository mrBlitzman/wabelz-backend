...
const validateOrder = async (formData, productData) => {
  ...
  const postData = async () => {
    try {
      const response = await axios.post(
        `${process.env.API_ORIGIN}/api/proceedOrder`,
        productData
      );
  ...
  if (!cleanedData.name || cleanedData.name.trim() === "") {
    ...
    cleanedData.name.trim().length < 3 ||
    cleanedData.name.trim().length > 50
  ) {
  ...
  if (!phone.trim()) {
    ...
    errors.phone =
      "Phone number format is invalid. It should include a country code and contain up " +
      "to 15 digits.";
  }
  ...
  if (!industry.trim()) {
    ...
    if (!validIndustries.includes(industry)) {
      errors.industry =
        "Invalid industry. Valid options are: Engineering, Marketing, Sales, Human Resour" +
        "ces, Design.";
    }
  }
  ...
  if (!websiteType.trim()) {
    ...
    if (!validWebsiteTypes.includes(websiteType)) {
      errors.websiteType =
        "Invalid website type. Valid options are: E-commerce, Blog, Portfolio, Corporate," +
        " Landing.";
    }
  }
